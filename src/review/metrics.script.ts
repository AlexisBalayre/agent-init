...
function parseSummary(raw: string): ReviewSummary | null {
  const parsed = parseReviewSummary(raw);
  if (parsed === null) return null;
  if ("rejected" in parsed) {
    warn(`${parsed.rejected}; recording it as absent`);
    return null;
  }
...
function postedCount(path: string | undefined): number | null {
  ...
  try {
    const posted = asRecord(JSON.parse(readFileSync(path, "utf8")))?.comments_posted;
    if (Number.isInteger(posted)) return posted as number;
    warn("posted.json has no integer comments_posted");
  } catch {
...
export function main(): void {
  ...
  if (env.GITHUB_STEP_SUMMARY) {
    appendFileSync(env.GITHUB_STEP_SUMMARY, summaryTable(record));
  }
}
