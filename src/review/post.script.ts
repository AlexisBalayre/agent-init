...
function fileDiff(mergeBase: string, file: string): string {
  try {
    // `:/` anchors the pathspec at the repo root: from any other directory a plain
    // repo-relative path would match nothing and quietly send every finding to the
    // summary instead of the diff.
