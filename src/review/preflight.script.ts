...
import { decideMode, newestCleanRecord, type PriorRecord, porcelainPaths } from "./preflight.utils.js";
...
function priorRecords(prNumber: string): PriorRecord[] {
  let hits: string;
  try {
    // `:(top)` anchors the pathspec at the repo root, wherever the CLI was started from.
