...
4. **Follow the project's writing conventions in the output.**
...
Source files (in the project's languages) that no other file imports, requires, or includes.
...
In a multi-package repo (workspaces, modules, crates), a shared package's public symbol that no consuming package uses. This is the **package-scoped search blind spot**: a search started inside the package misses the consumer side. Always grep from the repo root, not from inside the package.
...
5. **Cross-package check.** For a shared-package candidate, grep each consuming package separately. A package-scoped search is not enough.
...
- **Generated code:** codegen output, generated clients and stubs, anything a generator owns; regenerate, never report.
...
End with: "Verify each row before removing, then confirm with the project's typecheck and test commands (the project map in `AGENTS.md` names them). Remove one category per branch; do not bundle removals across categories in a single PR."
