...
> agentspine's `ci-review` pack emits: its preflight checks out the PR head and resolves the
> mode, it invokes `/pr-ci-review` with the PR named in prose plus the schema from
> `agentspine review schema`, `agentspine review post` renders and posts the record from a
