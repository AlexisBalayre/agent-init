...
> by [`claude-code-review.yml`](../../../.github/workflows/claude-code-review.yml), which
> agent-init's `ci-review` pack emits: its preflight checks out the PR head and resolves the
> mode, it invokes `/pr-ci-review` with the PR named in prose plus the schema from
> `agent-init review schema`, `agent-init review post` renders and posts the record from a
> separate job holding the write permissions, and each run's record lands on the
> `ci/review-metrics` orphan branch. Follow the setup comment at the
> top of that workflow to enable it; without it, the local `report` / `--fix` paths work
> unchanged. The CI path is Claude Code only; on other hosts, use `review-changes`.
...
- **Leave linters alone:** do not raise anything the project's formatter, linter, or typechecker (the project map in `AGENTS.md` names them, and `.agents/quality.toml` runs them) already catches; that is noise, not a finding.
...
- **local**: the working-tree diff against the merge-base with the trunk (the project map in `AGENTS.md` names it, default `main`): committed + staged + unstaged.
...
- **Under CI (a schema was requested):** the preflight step already asserted the working tree is the PR's clean current head, resolved the merge base, and decided the mode; had any of that failed, you would not be running. Two gates remain yours:
  - **The config-restore set.** After the preflight and before you started, the CI action replaced `.claude/`, `.mcp.json`, `.claude.json`, `.gitmodules`, `.ripgreprc`, `CLAUDE.md`, `CLAUDE.local.md`, `.husky`, and (through the workflow's own step) `AGENTS.md` and `.agents/` with the base branch's copies, because the head's versions are config the CLI executes at startup. A PR touching any of them therefore shows dirty exactly there; never `git checkout --` those paths (that re-arms the injection the restore disarms). The paths above the workflow's own step restores (`AGENTS.md`, `.agents/`, and whatever else its **`RESTORE_PATHS`** names) vary per repository: it covers what the shared tree resolves through, so a repository that symlinks `.agents/` into another directory restores that directory too, while the rest of the list is fixed by the action. Read that list before reading any file it names, and brief every reviewer with it: in-tree those paths hold the base branch's bytes, so a reviewer reading one and reporting what the PR "left unchanged" is reporting the base branch. Read the PR's own copies from `.claude-pr/<path>`, which the restore preserves unexecuted; commit-to-commit diffs (`gh pr diff`, `git diff <merge_base>..HEAD`) show those files' real changes either way.
...
- **Pre-flight (local source):** run the project's lint, typecheck, and test commands (the project map in `AGENTS.md` names them) **scoped to the affected packages or modules** where the runner supports it (the full suite is slow). A check the project does not have is skipped. If it fails, **stop**: report the failures and ask the user to return once the tree is green. Reviewers assume the deterministic layer is clean; spawning them on a red tree pays opus to rediscover what tooling already flagged. Integration tests are infra-gated and stay CI's job.
...
Do not discover instruction files: `AGENTS.md` is always in context (through `CLAUDE.md`), and its project map says where the convention docs and ADRs live; the conventions reviewer reads them.
...
| `review-conventions` | sonnet | almost always: the project's convention docs, ADRs, and `AGENTS.md` govern the rules |
...
**Set every model at spawn time**, reviewers and validators alike. The shipped manifests carry no `model:` field, because the key does not mean the same thing on every host they are installed for, so a spawn without an explicit model inherits yours and silently runs `correctness` and `security` on the orchestrator's tier.
...
- **Assign one area, citation-first.** A real defect is `correctness` or `security`. Anything citing a written rule from `AGENTS.md`, the project's convention docs, or an ADR is `conventions`. Remaining judgment calls are `docs` or `maintainability`.
...
- **Post-verify**: after editing, re-run the project's typecheck and test commands (scoped to the affected area where the runner allows) to prove no regression was introduced. Report the result.
