...
- **Hardcode what is stable, look up what is not.** A document may bake in a workflow that holds for every project it ships to, but anything project- or machine-specific (doc paths, lint/typecheck/test commands, tracker or vendor identifiers) is read from the project map in `AGENTS.md` or asked for, never assumed. A document that hardcodes one repository's layout is not portable, and one that hardcodes a person's tooling leaks it.
- **Point at the project's own docs, never a parallel store.** Reference whatever glossary, conventions, narrative docs and decision log the project already keeps, the way `domain-modeling` does. Never create a private `CONTEXT.md` that duplicates them and immediately starts to rot.
- **`AGENTS.md` is tool-neutral; a tool-specific file adds only what is tool-specific.** `CLAUDE.md` imports `AGENTS.md` (`@AGENTS.md`); Codex, opencode, Mistral Vibe and Cursor read `AGENTS.md` directly. Neither layer restates the other.
- **Skills live once, in `.agents/skills/`.** Each tool reaches that directory natively or through a symlink; edit the canonical copy, never a per-tool one.
