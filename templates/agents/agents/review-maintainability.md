...
- A single-caller split a change makes to satisfy a CI-enforced gate (a cognitive-complexity or function-length rule, read from the stated intent) is the mechanism the PR uses, not a regression; flag it only when inlining it back would not re-trip the gate.
