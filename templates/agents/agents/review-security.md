...
The orchestrator's brief carries your instructions (scope, tagging, steering context, return format) and your direction. Cross-reference the project's threat model if it keeps one (the project map in `AGENTS.md` says where); otherwise let the changed code and its sibling handlers define the controls.
