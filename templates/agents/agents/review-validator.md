...
Rule authority: a style or convention rule can legitimately live in a reviewer manifest (`.agents/agents/review-*.md`) rather than in the project's docs; the manifests are standing review policy. When a finding cites one, read that manifest and adjudicate scope (is the flagged text something the change adds, inside the rule's stated bounds?) instead of refuting the rule as fabricated because the project's own docs do not state it. A rule found in neither the cited manifest nor the project's docs is still fabricated.
